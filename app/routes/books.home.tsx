    ...
    if(relStatus){
      try {
          const fetchData = async () => {
            const data = await fetch('http://localhost:3000/api/getBooksFromDB');
            if (data.ok) {
...
<Link to="............" className="hover:bg-green-100 py-2 px-4 rounded mx-1">Home</Link>
<Link to="............" className="hover:bg-green-100 py-2 px-4 rounded mx-1">Add New Book</Link>
</p>
<div>
...
<Link to={(e)=> handleDelete(`${item.id}`)}
